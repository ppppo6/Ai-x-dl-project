...
| 변수 | 변수명 | 설명 |
| --- | --- | --- |
| 기온 | Temperature | 그 지역의 기온을 나타내며, 기온이 높으면 패널 효율이 떨어져 발전량이 낮아질 수도 있다. <br> 단위는 °C 이다. |
| 맑은 하늘 기준 일사량 | Clearsky GHI | 인버터를 통해 변환된 실제 교류(AC) 출력 전력의 평균값이다. 1분마다 측정되었으며 단위는 kW이다. |
| 실제 일사량 | GHI | 인버터를 통해 변환된 실제 교류(AC) 출력 전력의 평균값이다. 1분마다 측정되었으며 단위는 kW이다. |
| 상대 습도 | Relative Humidity | 인버터를 통해 변환된 실제 교류(AC) 출력 전력의 평균값이다. 1분마다 측정되었으며 단위는 kW이다. |
| 기압 | Pressure | 인버터를 통해 변환된 실제 교류(AC) 출력 전력의 평균값이다. 1분마다 측정되었으며 단위는 kW이다. |
| 풍속 | Wind Speed | 인버터를 통해 변환된 실제 교류(AC) 출력 전력의 평균값이다. 1분마다 측정되었으며 단위는 kW이다. |
