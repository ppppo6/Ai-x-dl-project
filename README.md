...
이번 프로젝트를 통해 시계열 데이터 예측 문제에서는 최신 모델이라고 해서 항상 가장 좋은 성능을 보이는 것은 아니며, 데이터의 특성과 규모에 맞는 모델을 선택하는 것이 중요하다는 점을 확인할 수 있었습니다. 향후 더 많은 기간의 데이터를 확보하고 하이퍼파라미터를 최적화한다면 Transformer의 성능을 더욱 향상시킬 수 있을 것으로 기대됩니다. 또한 다양한 기상 변수와 외부 환경 요인을 추가로 활용한다면 태양광 발전량 예측의 정확도를 높일 수 있을 것으로 생각됩니다.
