...
하지만 최근 RNN은 순차적 데이터를 처리하는 데 훨씬 더 효율적인 트랜스포머 기반 AI와 대규모 언어 모델(LLM)들로 대체되고 있습니다. 왜냐하면 RNN 모델은 치명적인 단점이 하나 존재하기 때문입니다. RNN은 시퀀스 길이가 길어질수록 초기 정보가 점차 사라지는 기울기 소실(Vanishing Gradient) 문제가 발생할 수 있습니다. 기울기 소실 문제는 모델이 훈련을 거듭할수록 기울기가 0에 가까워지는 상태를 뜻합니다. 기울기가 0에 가까워지 RNN이 훈련 데이터를 효과적으로 학습하지 못해 과소적합(과적합의 반대상황)이 일어날 수 있습니다. 이러한 문제로 인해 시퀀스가 길어지면 앞부분의 내용을 까먹는 장기 의존성 문제(Long-Term Dependency)가 생길 수도 있습니다.
