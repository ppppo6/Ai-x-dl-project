...
#### 첫 번째로 관측기간이 4개월로 매우 짧다는 것입니다. 관측 기간이 짧으면 RNN, LSTM, Transformer 모델들 사이의 차이가 두드러지게 안 날 것입니다. 두 번째로 이 데이터셋은 직접적인 태양광 발전량에 관한 내용이 없다는 것입니다. 대신에 일사량이 있긴 하고 이것이 태양광 발전량과 큰 관련이 있긴 하지만 정확히 발전량을 대변하긴 힘듭니다.  
#### 따라서 현재 다른 많은 데이터셋들을 찾아보고 있는 상태입니다. 저희가 원하는 조건들 (3년치 이상, 기상 상황/태양광 발전량 모두 포함/단일 지역에서 측정 etc..) 을 만족시키는 데이터셋을 찾기 힘들더라고요,,
#### 현재 두 개의 데이터셋을 찾아서 합치는 방법도 고려중입니다. 
#### https://re.jrc.ec.europa.eu/pvg_tools/en/ 나 https://nsrdb.nlr.gov/data-viewer 등 여러 사이트 탐색중입니다.
...
Transformer 관련 내용 :  강의자료 07-beyond 참
