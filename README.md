...
- 원래는 밤에는 햇빛이 없기 때문에 0으로 나오는게 맞다. 하지만 센서 노이즈같은 이유때문에 발전량이 음수로 나오는 경우가 있다. 이런 문제를 해결하기 위해 음수값을 모두 0으로 처리했다.
...
- 발전량이 0.68777777777 같이 소수점 3~4자리 이후부터는 무의미하게 나오는 경우가 많아서 깔끔한 데이터 처리를 위해 반올림했다. (실제로도 큰 영향을 끼치지 않음)
