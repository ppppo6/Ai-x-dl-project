...
**X_train.npy : 학습용 입력 변수 데이터 (shape: 13880, 168, 6)**

13880개의 샘플이 있으며, 각 샘플은 과거 168개 시점의 기상 변수 6개로 구성되었습니다.

**X_test.npy : 평가용 입력 변수 데이터 (shape: 3471, 168, 6)**

3471개의 샘플이 있으며, 각 샘플은 과거 168개 시점의 기상 변수 6개로 구성되었습니다.

이것은 모델이 테스트를 거칠 때 처음 보는 데이터로 성능 평가에 사용됩니다.

**Y_train.npy : 학습용 정답 데이터 (shape: 13880, 1)**

13880개의 샘플이 있으며, 각 샘플은 X_train의 각 샘플에 대응하는 실제 발전량 값입니다.

**Y_test.npy : 평가용 정답 데이터 (shape: 3471, 1)**

3471개의 샘플이 있으며, 각 샘플은 X_test의 각 샘플에 대응하는 실제 발전량 값입니다.

**scaler_X.pkl : 입력 변수 6개를 정규화할 때 사용한 MinMaxScaler 저장 파일**

**scaler_Y.pkl : 발전량을 정규화할 때 사용한 MinMaxScaler 저장 파일**

+) 모델 예측값(0~1)을 실제 발전량(kW)으로 역정규화할 때 사용하기도 합니다.
