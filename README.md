...
이 데이터셋의 측정 간격은 1분이며, 총 6곳에서 측정한 데이터가 존재해서 저희가 원하는 1곳을 골라서 따로 데이터를 얻을 수 있습니다.
...
원래 이 데이터셋에는 발전량 이외에도 많은 컬럼들(ex. 장비 상태)이 있습니다.

하지만 저희가 여기서 필요했던 것은 발전량이였기 때문에 1개의 컬럼만 사용했습니다.

| 역할 | 변수명 | 설명 |
| --- | --- | --- |
| 발전 | InvPAC_kW_Avg | 인버터를 통해 변환된 실제 교류(AC) 출력 전력의 평균값이다. 1분마다 측정되었으며 단위는 kW이다. |
