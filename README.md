...
먼저 RNN의 경우 시계열 데이터를 순차적으로 처리할 수 있다는 장점이 있지만, 긴 시퀀스에서는 과거 정보를 충분히 기억하지 못하는 *기울기 소실(Vanishing Gradient)* 문제가 존재합니다. 본 실험에서 직접 기울기 소실을 측정할 수는 없었지만 Train Loss 감소량을 비교하여 일부 영향을 추정할 수 있었습니다. RNN의 Train Loss는 0.009847에서 0.008908로 감소한 반면, LSTM은 0.009534에서 0.007869까지 감소하였습니다. 이는 두 모델 모두 정상적으로 학습되었음을 의미하지만, LSTM이 더 많은 패턴을 학습하였음을 보여줍니다. 따라서 기울기 소실이 심각한 수준은 아니었으나 긴 시퀀스(168개 시점)를 처리하는 과정에서 일정 부분 영향을 끼쳤을 가능성이 있다고 판단하였습니다.
...
LSTM이 가장 좋은 성능을 보인 이유는 장기 의존성(Long-Term Dependency)을 효과적으로 학습할 수 있기 때문입니다. 본 프로젝트에서는 과거 168개 시점(약 3.5일)의 기상 데이터를 이용하여 다음 시점의 발전량을 예측하였습니다. LSTM은 셀 상태(Cell State)와 게이트(Gate) 구조를 이용하여 중요한 정보를 장기간 유지할 수 있기 때문에, 태양광 발전량과 기상 조건 사이의 복잡한 관계를 RNN보다 더 효과적으로 학습할 수 있었습니다. 그 결과 가장 낮은 MAE와 RMSE, 가장 높은 R² 값을 기록하였습니다.

한편 가장 흥미로운 결과는 Transformer 모델이었습니다. 일반적으로 Transformer는 RNN이나 LSTM보다 발전된 구조로 평가받으며 다양한 분야에서 우수한 성능을 보여주고 있습니다. 따라서 실험 전에는 Transformer가 가장 좋은 성능을 보일 것으로 예상하였습니다. 그러나 실제 결과에서는 LSTM보다 낮은 성능을 보였으며, RNN과 거의 비슷한 수준에 머물렀습니다.

Transformer의 Loss Curve를 살펴보면 Train Loss는 지속적으로 감소하였지만 Test Loss는 초반 이후 약 0.010 수준에서 크게 개선되지 않는 모습을 보였습니다. 이는 모델이 학습 데이터에 대해서는 점차 적합해지고 있지만 새로운 데이터에 대한 일반화 성능은 크게 향상되지 않았음을 의미합니다. 또한 일부 구간에서는 Test Loss가 다시 증가하는 모습을 보여 과적합이 진행되었을 가능성도 확인할 수 있었습니다.
...
결과적으로 Transformer가 RNN이나 LSTM보다 낮은 성능을 보인 것은 모델 자체의 성능 문제가 아니라 데이터 규모가 Transformer의 장점을 충분히 발휘하기에 부족했기 때문이라고 해석할 수 있습니다. 만약 수년 이상의 데이터 또는 여러 지역의 발전소 데이터를 함께 사용하였다면 Transformer의 성능이 더욱 향상될 가능성이 있다고 생각됩니다.

종합하면 본 프로젝트에서는 **LSTM > Transformer ≈ RNN**의 성능 순서가 나타났습니다. 이는 태양광 발전량 예측과 같이 데이터 규모가 상대적으로 제한적인 시계열 예측 문제에서는 장기 기억 능력을 갖춘 LSTM이 가장 효율적인 모델임을 보여줍니다. 또한 최신 모델이라고 해서 항상 더 좋은 성능을 보이는 것은 아니며, 모델의 복잡도와 데이터 규모 사이의 균형이 매우 중요하다는 사실을 확인할 수 있었습니다.
