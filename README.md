...
### !!위의 데이터셋은 Blog 진행과정을 위해 임시로 넣은 데이터셋입니다. 우선 이 데이터셋은 크게 2개의 문제점을 지닙니다. 첫 번째로 관측기간이 4개월로 매우 짧다는 것입니다. 관측 기간이 짧으면 RNN, LSTM, Transformer 모델들 사이의 차이가 두드러지게 안 날 것입니다. 두 번째로 이 데이터셋은 직접적인 태양광 발전량에 관한 내용이 없다는 것입니다. 대신에 일사량이 있긴 하고 이것이 태양광 발전량과 큰 관련이 있긴 하지만 
