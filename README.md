...
종합하자면 본 프로젝트에서는 **LSTM > Transformer ≈ RNN**의 성능 순서가 나타났습니다. 이는 태양광 발전량 예측과 같이 데이터 규모가 상대적으로 제한적인 시계열 예측 문제에서는 장기 기억 능력을 갖춘 **LSTM**이 가장 효율적인 모델임을 보여줍니다. 또한 최신 모델이라고 해서 항상 더 좋은 성능을 보이는 것은 아니며, 모델의 복잡도와 데이터 규모 사이의 균형이 매우 중요하다는 사실을 확인할 수 있었습니다.
